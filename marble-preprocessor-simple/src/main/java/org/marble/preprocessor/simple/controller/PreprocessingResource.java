package org.marble.preprocessor.simple.controller;
...
import org.marble.preprocessor.simple.service.PreprocessorService;
...
public class PreprocessingResource {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(PreprocessingResource.class);
    ...
    public Response process(ProcessorInput input, @Context UriInfo uriInfo) {
        ...
        output.setMessage(preProcessorService.preprocessMessage(input.getMessage()));
        output.setNotes("Processed message <"+input.getMessage()+"> using simple preprocessor.");
