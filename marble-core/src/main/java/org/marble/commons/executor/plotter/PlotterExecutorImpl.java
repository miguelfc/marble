...
public class PlotterExecutorImpl implements PlotterExecutor {
  ...
  public void run() {
    ...
    try {
      ...
      logMsg("An error ocurred while plotting posts with execution <" + execution.getId()
          + ">. Execution aborted.", "error", e);
  ...
  private void plot() throws InvalidExecutionException {
    ...
    for (JobParameters parameter : this.execution.getParameters()) {
      ...
      CompletableFuture<Response> promise = asyncHttpClient.preparePost(url)
          .setBody(gson.toJson(input)).setHeader("Content-Type", "application/json").execute()
          .toCompletableFuture().thenApply(resp -> {
            try {
              ...
              if (output != null) {
                ...
                for (Chart chart : temporaryChartsList) {
                  ...
                  // By convention, the figures list coming from the plotter contains the figures in
                  // base64.
            ...
            logMsg("An error occurred while using plotter.", "error", t);
            return null;
          });
